...
/**
 * Servlet implementation class GetCTATActivity
 * @author cskamil
 */
@WebServlet("/GetCTATActivity")
public class GetCTATActivity extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	public GetCTATActivity() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashMap<String, String[]> ctat_activity = this.getCTATActivity(contentInput.getUserId(), contentInput.getGroudId(), contentInput.getDateFrom());
		...
		for (ProviderContent providerContent : providerContentList) {
			...
			if(contentList.length != 0) {
				for (String content : contentList) {
					ContentProgressSummary progressSummary = createProgressSummary(content, ctat_activity);
					progressOutput.addContentProgress(progressSummary);
				}
			} else {
				Set<Entry<String, String[]>> activitySet = ctat_activity.entrySet();
				for (Entry<String, String[]> contentActivity : activitySet) {
					ContentProgressSummary progressSummary = createProgressSummary(contentActivity.getKey(), ctat_activity);
					progressOutput.addContentProgress(progressSummary);
				}
			}
	...
	public HashMap<String, String[]> getCTATActivity(String usr, String domain, String dateFrom) {
		ConfigManager cm = new ConfigManager(this);
		um2DBInterface um2_db = new um2DBInterface(cm.um2_dbstring,cm.um2_dbuser,cm.um2_dbpass);
		
		um2_db.openConnection();
		HashMap<String, String[]> ctatActivityMap= um2_db.getUserCTATActivity(usr, dateFrom);
		um2_db.closeConnection();
		
		return ctatActivityMap;
	}
}
