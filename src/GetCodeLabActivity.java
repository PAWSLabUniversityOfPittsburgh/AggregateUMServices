...
/**
 * Servlet implementation class GetCodeLabActivity
 * @author cskamil
 */
@WebServlet("/GetCodeLabActivity")
public class GetCodeLabActivity extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	public GetCodeLabActivity() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HashMap<String, String[]> codelab_activity = this.getCodeLabActivity(contentInput.getUserId(), contentInput.getGroudId(), contentInput.getDateFrom());
		...
		for (ProviderContent providerContent : providerContentList) {
			...
			if(contentList.length != 0) {
				for (String content : contentList) {
					ContentProgressSummary progressSummary = createProgressSummary(content, codelab_activity);
					progressOutput.addContentProgress(progressSummary);
				}
			} else {
				Set<Entry<String, String[]>> activitySet = codelab_activity.entrySet();
				for (Entry<String, String[]> contentActivity : activitySet) {
					ContentProgressSummary progressSummary = createProgressSummary(contentActivity.getKey(), codelab_activity);
					progressOutput.addContentProgress(progressSummary);
				}
			}
	...
	public HashMap<String, String[]> getCodeLabActivity(String usr, String domain, String dateFrom) {
		ConfigManager cm = new ConfigManager(this);
		um2DBInterface um2_db = new um2DBInterface(cm.um2_dbstring,cm.um2_dbuser,cm.um2_dbpass);
		
		um2_db.openConnection();
		HashMap<String, String[]> codeLabActivityMap= um2_db.getUserCodeLabActivity(usr, dateFrom);
		um2_db.closeConnection();
		
		return codeLabActivityMap;
	}
}
